...
const pico = new APIClient({
    provider: new MockProvider(joinPath(__dirname, 'data'), 'https://pico.greymass.com'),
})
...
const bpico = new APIClient({
    provider: new MockProvider(joinPath(__dirname, 'data'), 'https://api.bpico.world'),
})
...
suite('api v1', function () {
    ...
    test('chain get_account (system account)', async function () {
        const account = await jungle.v1.chain.get_account('picoio')
        assert.equal(account.account_name, 'picoio')
    })
    ...
    test('chain get_block (by id)', async function () {
        const block = await pico.v1.chain.get_block(
    ...
    test('chain get_block (by num)', async function () {
        const block = await pico.v1.chain.get_block(8482113)
        assert.equal(block.block_num, 8482113)
        assert.equal(
            block.id.hexString,
            '00816d41e41f1462acb648b810b20f152d944fabd79aaff31c9f50102e4e5db9'
    ...
    test('chain get_block w/ new_producers', async function () {
        const block = await pico.v1.chain.get_block(92565371)
        assert.equal(block.block_num, 92565371)
    })

    test('chain get_block w/ transactions', async function () {
        const block = await pico.v1.chain.get_block(124472078)
    ...
    test('chain get_block_header_state', async function () {
        const header = await pico.v1.chain.get_block_header_state(143671483)
    ...
    test('chain get_currency_balance', async function () {
        const balances = await jungle.v1.chain.get_currency_balance('picoio.token', 'lioninjungle')
        assert.equal(balances.length, 2)
        balances.forEach((asset) => {
            assert.equal(asset instanceof Asset, true)
        })
        assert.deepEqual(balances.map(String), ['881307350.1453 PICO', '100400.0000 JUNGLE'])
    })
    ...
    test('chain get_info (bpico)', async function () {
        const info = await bpico.v1.chain.get_info()
        assert.equal(
            info.chain_id.hexString,
    ...
    test('chain push_transaction', async function () {
        ...
        const action = Action.from({
            ...
            data: Transfer.from({
                from: 'corecorecore',
                to: 'teamgreymass',
                quantity: '0.0042 PICO',
                memo: 'picoio-core is the best <3',
            }),
        })
    ...
    test('chain get_table_rows (untyped)', async function () {
        const res = await pico.v1.chain.get_table_rows({
            code: 'picoio.token',
            table: 'stat',
            scope: Asset.Symbol.from('4,PICO').code.value.toString(),
            key_type: 'i64',
        })
        assert.equal(res.rows.length, 1)
        assert.equal(res.rows[0].max_supply, '10000000000.0000 PICO')
    })
    ...
    test('chain get_table_rows (typed)', async function () {
        ...
        const res1 = await pico.v1.chain.get_table_rows({
            code: 'fuel.gm',
            table: 'users',
            type: User,
            limit: 1,
        ...
        const res2 = await pico.v1.chain.get_table_rows({
            code: 'fuel.gm',
            table: 'users',
            type: User,
            limit: 2,
